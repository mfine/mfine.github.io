...
p. Amazon's Simple Notification Service enables you to publish messages to subscribers in a scalable, cost-effective manner. Presently, notifications can be delivered either to email or HTTP endpoints. This post helps get you started setting up an embedded HTTP server and then processing notifications in Java using the "AWS SDK for Java":http://aws.amazon.com/sdkforjava/, the "Jetty HTTP Server":http://jetty.codehaus.org/jetty/, and the  "Jackson JSON processor":http://jackson.codehaus.org/.
...
p. The complete code for both senders and receivers can be found is at "AmazonSNSExample":http://github.com/mfine/AmazonSNSExample.
